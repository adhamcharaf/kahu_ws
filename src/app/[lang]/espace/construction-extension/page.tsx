...
interface ConstructionExtensionPageProps {
  params: Promise<{ lang: string }>;
}

export async function generateMetadata({ params }: ConstructionExtensionPageProps) {
  const { lang: langParam } = await params;
  const lang = (isValidLocale(langParam) ? langParam : 'fr') as Locale;
  const dict = await getDictionary(lang);

  return {
    title: dict.espace.constructionExtension.title,
    description: dict.espace.constructionExtension.heroText,
  };
}
...
export default async function ConstructionExtensionPage({ params }: ConstructionExtensionPageProps) {
  const { lang: langParam } = await params;
  const lang = (isValidLocale(langParam) ? langParam : 'fr') as Locale;
  const dict = await getDictionary(lang);
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <SectionReveal>
            ...
            <h1 className="font-display text-display-xl text-kahu-terracotta uppercase tracking-[0.1em]">
              {dict.espace.constructionExtension.title}
            </h1>
            <p className="mt-6 text-body-lg text-kahu-taupe max-w-2xl">
              {dict.espace.constructionExtension.heroText}
            </p>
          </SectionReveal>
          ...
          <SectionReveal delay={0.2}>
            <ul className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl">
              {dict.espace.constructionExtension.features.map((feature, index) => (
