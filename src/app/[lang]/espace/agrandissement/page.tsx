...
interface AgrandissementPageProps {
  params: Promise<{ lang: string }>;
}

export async function generateMetadata({ params }: AgrandissementPageProps) {
  const { lang: langParam } = await params;
  const lang = (isValidLocale(langParam) ? langParam : 'fr') as Locale;
  const dict = await getDictionary(lang);

  return {
    title: dict.espace.agrandissement.title,
    description: dict.espace.agrandissement.heroText,
  };
}
...
async function ProjectsGrid({ lang }: { lang: Locale }) {
  ...
  // const filteredProjects = projects.filter(p => p.type === 'agrandissement');
    ...
    return (
      <div className="text-center py-16">
        <p className="text-body-md text-kahu-taupe">
          Aucun projet d'agrandissement disponible pour le moment.
        </p>
      </div>
    );
  }
...
export default async function AgrandissementPage({ params }: AgrandissementPageProps) {
  const { lang: langParam } = await params;
  const lang = (isValidLocale(langParam) ? langParam : 'fr') as Locale;
  const dict = await getDictionary(lang);
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <SectionReveal>
            ...
            <p className="mt-6 text-body-lg text-kahu-taupe max-w-2xl">
              {dict.espace.agrandissement.heroText}
            </p>
          </SectionReveal>
          ...
          <SectionReveal delay={0.2}>
            <ul className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl">
              {dict.espace.agrandissement.features.map((feature, index) => (
